...
(function(a) {
    ...
    function m(c, d) {
        var b = a(this).find("[data-valmsg-for='" + f(d[0].name) + "']"), e = a.parseJSON(b.attr("data-valmsg-replace")) !== false;
    ...
    function i(c) {
        ...
        if (!d) {
            d = {
                options: { errorClass: "input-validation-error", errorElement: "span", errorPlacement: a.proxy(m, c), invalidHandler: a.proxy(l, c), messages: { }, rules: { }, success: a.proxy(k, c) },
                attachValidation: function() { b.unbind("reset." + e, f).bind("reset." + e, f).validate(this.options); },
                validate: function() {
                    b.validate();
                    return b.valid();
                }
    ...
    d.unobtrusive = {
        ...
        parse: function(b) {
            ...
            a(b).find(":input[data-val=true]").each(function() { d.unobtrusive.parseElement(this, true); });
            c.each(function() {
                var a = i(this);
    ...
    b.add("remote", ["url", "type", "additionalfields"], function(b) {
        ...
        a.each(j(b.params.additionalfields || b.element.name), function(i, h) {
            var c = g(h, e);
            d.data[c] = function() { return a(b.form).find(":input[name='" + f(c) + "']").val(); };
        });
        c(b, "remote", d);
    });
